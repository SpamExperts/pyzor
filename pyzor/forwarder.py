...
class Forwarder(object):
    ...
    def __init__(self,forwarding_client,remote_servers,max_queue_size=10000):
        """
        forward_client: a pyzor.client.Client instance to use as forwarding client
        remote_servers: a list of (hostname,port) tuples where digests should be forwarded to
        ...
        while True:
            ...
            for server in self.remote_servers:
                try:
                    ...
                    self.log.warn('Forwarding digest %s to %s failed: %s'%(digest,server,str(e)))
